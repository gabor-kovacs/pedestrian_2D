...
class GLViewer:
    def __init__(self):
        ...
        # !
        self.pedestrians = Simple3DObject(False, 3, 4)
        self.orientations = Simple3DObject(False, 3, 4)
    ...
    def init(self, _argc, _argv, camera_model, res):  # _params = sl.CameraParameters
        ...
        self.bckgrnd_clr = np.array([30/255., 30/255., 30/255.])
        ...
        if(camera_model == sl.MODEL.ZED):
            ...
            for i in range(0, zm.NB_AL_ZEDM_TRI * 3, 3):
                for j in range(3):
                    ...
                    self.zedModel.add_point_clr([zm.vertices_m[index * 3], zm.vertices_m[index * 3 + 1], zm.vertices_m[index * 3 + 2]],
        ...
        self.pedestrians.set_drawing_type(GL_POINTS)
        self.orientations.set_drawing_type(GL_LINES)
    ...
    def updateData(self, pc, peds):
        ...
        for ped in peds:
            color = generate_color_from_id(ped.id)
            self.pedestrians.add_point_clr([ped.x, ped.y, ped.z], color)

            # add line to represent pedestiran orientation

            # vector_length = 1.0
            correction_angle = math.atan(-1. * ped.x / ped.z)

            ori = math.radians(ped.orientation - correction_angle)
            ori_x = -1. * math.sin(ori)
            ori_z = -1. * math.cos(ori)

            # print(f"{ped.id}: {ped.orientation}")
            # print(f"{ped.id} correction angle: {math.degrees(correction_angle)}")

            self.orientations.add_line([ped.x, ped.y, ped.z], [ped.x + ori_x, ped.y, ped.z + ori_z], color)

            # control point to 100 0 100
            # self.pedestrians.add_point_clr([2.0,0.0,2.0], [1.,1.,1.])
            # self.pedestrians.add_point_clr([2.0,0.0,2.0], [1.,1.,1.])
    ...
    def update(self):
        if(self.mouse_button[0]):
            ...
            r.init_angle_translation(self.mouseMotion[0] * -0.01, vert)
            self.camera.rotate(r)

            r.init_angle_translation(self.mouseMotion[1] * -0.01, self.camera.right_)
        ...
        # !
        self.pedestrians.push_to_GPU()
        self.orientations.push_to_GPU()
    ...
    def draw(self):
        ...
        glPointSize(14.2)
        glLineWidth(3.2)
        self.pedestrians.draw()
        self.orientations.draw()
        glUseProgram(0)
