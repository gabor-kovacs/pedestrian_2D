...
class Tracker:
    def __init__(self, save_old_tracks, max_age, max_dist_iou, max_dist_feature, n_init, reinit_interval, point_termi, start_ind, max_size, thre_conf, thre_var_ratio,
                 thre_homo, point_detect, focus_point_manual, focus_point_auto, use_mask, head_detect, r_ratio, interval_num, K, max_point_num, shi_tomasi, feature_params, lk_params):
        ...
        # * save the old tracks so we can plot later
        self.save_old_tracks = save_old_tracks
        self.old_tracks = []
...
#                        print("track_id", self.tracks[i].track_id, "mark deleted", determ)
        ...
        # save deleted tracks
        if self.save_old_tracks:
            for t in self.tracks:
                if t.is_deleted:
                    self.old_tracks.append(t)
