...
def main():
    ...
    tracks_resolution = sl.Resolution(int(display_resolution.height * 0.96568 * 2.0), display_resolution.height )
    ...
    track_view_generator = cv_viewer.TrackingViewer(tracks_resolution, camera_config.camera_fps, 20.0)
    ...
    while (quit_app == False):
        if zed.grab(runtime_parameters) == sl.ERROR_CODE.SUCCESS:
            ...
            if opts.show_output_images:
                ...
                colorized_preds2 = np.copy(colorized_preds)
            ...
            for track in tracker.tracks:
                ...
                zeros = np.zeros([mask_img.shape[0], mask_img.shape[1]], np.uint8)
                # convert grayscale to rgb

                mask_img_rgb = np.stack((zeros, zeros, mask_img), axis=2)
                ...
                erosion_rgb = np.stack((zeros, zeros, erosion), axis=2)

                # print the output
                """
                ped_img = frame[top:bottom, left:right]
                cv2.imshow("ped_img", ped_img)
                cv2.imshow("mask_img_rgb", mask_img_rgb)
                cv2.imshow("erosion_rgb", erosion_rgb)
                cv2.addWeighted(ped_img,0.85,mask_img_rgb,0.15,0,ped_img)
                cv2.addWeighted(ped_img,0.85,erosion_rgb,0.15,0,ped_img)
                cv2.imshow("erosion", ped_img)
                cv2.imwrite("labels.png", colorized_preds[top:bottom, left:right])
                cv2.imwrite("mask_img.png", mask_img)
                cv2.imwrite("erosion.png", erosion)
                cv2.imwrite("overlay.png", ped_img)
                """
                ...
                # position without mask
                x_median, y_median, z_median = helpers.get_3d_position_from_xyz(xyz,top,bottom,left,right)
                # print(f"without masking: {x_median}, {y_median}, {z_median}")
                # position with mask
                x_median, y_median, z_median = helpers.get_3d_position_from_xyz(xyz,top,bottom,left,right,mask_img)
                # print(f"with masking: {x_median}, {y_median}, {z_median}")
                

                """
                # ! show masked image
                ped = frame[top:bottom, left:right]
                cv2.imshow("ped",frame[top:bottom, left:right])
                ped_masked = cv2.bitwise_and(ped, ped, mask = mask_img)
                cv2.imshow("ped_masked", ped_masked)
                cv2.waitKey(0)
                """
            ...
            if opts.show_output_images:
                for track in tracker.tracks:
                    ...
                    bbox = [int(i) for i in track.bbox]
                    # draw bounding box
                    cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[0]+bbox[2], bbox[1]+bbox[3]), generate_color_from_id(track_id), thickness=2)
                    cv2.rectangle(colorized_preds, (bbox[0], bbox[1]), (bbox[0]+bbox[2], bbox[1]+bbox[3]), generate_color_from_id(track_id), thickness=2)
                    # draw foot area
                    cv2.rectangle(frame, (bbox[0], bbox[1] + bbox[3] - math.floor(0.5 * bbox[2])), (bbox[0]+bbox[2], bbox[1]+bbox[3]), generate_color_from_id(track_id), thickness=1)
                    cv2.rectangle(colorized_preds, (bbox[0], bbox[1] + bbox[3] - math.floor(0.5 * bbox[2])), (bbox[0]+bbox[2], bbox[1]+bbox[3]), generate_color_from_id(track_id), thickness=1)
                    # draw id
                    cv2.putText(frame, str(track_id), (bbox[0], bbox[1] - 5), cv2.FONT_HERSHEY_PLAIN, 1,generate_color_from_id(track_id), thickness=2)
                    cv2.putText(colorized_preds, str(track_id), (bbox[0], bbox[1] - 5), cv2.FONT_HERSHEY_PLAIN, 1, generate_color_from_id(track_id), thickness=2)
                    for point in track.points:
                        cv2.circle(frame, (int(point[0]), int(point[1])), 3, generate_color_from_id(track_id), -1)
            ...
            if svo_position == 231:
                for track in tracker.tracks:
                    ...
                    if left < 0: 
                        left = 0
                        right = int(0 + track.bbox[2])
                    crop_img = frame2[top:bottom, left:right]
                    cv2.imwrite(f"../paper/images/ped_{id}.jpg", crop_img)
                    crop_leg = frame2[bottom - math.floor(width * 0.5):bottom, left:right]
                    cv2.imwrite(f"../paper/images/ped_leg_{id}.jpg", crop_leg)
                    crop_leg_seg = colorized_preds2[bottom - math.floor(width * 0.5):bottom, left:right]
                    cv2.imwrite(f"../paper/images/ped_leg_seg_{id}.jpg", crop_leg_seg)
                    print(f"LOCATION: {track.location_level}")

                cv2.imwrite("../paper/images/frame.jpg", frame)
                cv2.imwrite("../paper/images/colorized_preds.jpg", colorized_preds)
                cv2.imwrite("../paper/images/image_track_ocv.jpg", image_track_ocv)
                cv2.imwrite("../paper/images/image_left_ocv.jpg", image_left_ocv)
                    ...
                    if PLOT:
                        for track in tracker.old_tracks:
                            if track.track_id == 1:
                                ...
                                # axs[0].set_xlim([120, 320])
                                # axs[1].set_xlim([120, 320])
                                ...
                                axs[0].plot(track.frames, track.collision_distances, label = "Collision distance (d)", color="black")
                                axs[1].plot(track.frames, track.safety_levels, label = "Safety level (s)")
                                axs[1].plot(track.frames, track.awareness_levels, label = "Awareness level (s_gaze)")
                                axs[1].plot(track.frames, track.collision_levels, label = "Distance safety level (s_dist)")
                                axs[1].plot(track.frames, track.location_levels, label = "Location safety level (s_loc)")
                                ...
                                f.write("frame,d,s,s_gaze,s_dist,s_loc\n")
                                for line in zip(track.frames, 
                                                track.collision_distances, 
                                                track.safety_levels, 
                                                track.awareness_levels,
                                                track.collision_levels,
                                                track.location_levels):
